...
//now do what ever initialization is needed
  ...
  mtree->GetBranch("electron_disp")->SetTitle("electron displacement from primary vertex (um)");
...
{*/
  if( itElec1->pt()>10 ){
    ...
    for(TrackCollection::const_iterator itTrack2 = tracks->begin();
       ...
       {
       if( itTrack2->pt()>5 ){
        //if(itTrack2->charge()==1 || itTrack2->charge()==-1){ if tracks electron init

         
         KalmanVertexFitter fitter;
         vector<TransientTrack> trackVec;
         if(t_tks.size()>2 && itElec1->pt()!=itTrack2->pt()){

           auto trk1 = itElec1->gsfTrack();
           //cout<<"esta: "<<trk1->trackerExpectedHitsInner().numberOfHits()<<endl;
           TransientTrack t_trk1 = (*theB).build(trk1);

           //if(t_trk1.isValid()){ cout<<"esta2: "<<t_trk1.stateAtBeamLine().transverseImpactParameter().value()<<endl;}


           /*auto trk2 = itElec2->gsfTrack();
           TransientTrack t_trk2 = (*theB).build(trk2);*/
           ...
           if(myVertex.isValid()){
             //primaryvertex
             cout<<"pt1: "<<itElec1->pt()<<" c1: "<<itElec1->charge()<<" pt2: "<<itTrack2->pt()<<" c2: "<<itTrack2->charge()<<endl;
             ...
             disp= sqrt(dispx*dispx + dispy*dispy + dispz*dispz)/err;             
             ...
             cout<<"DeltaR: "<<DeltaR<<"\n\n";
             electron_deltaR.push_back(DeltaR);
             if(i==0){electron_deltaR1.push_back(DeltaR);}
             else if(i==1){electron_deltaR2.push_back(DeltaR);}
           }
